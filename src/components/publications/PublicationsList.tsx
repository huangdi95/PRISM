...
// const HIGHLIGHT_KEYWORDS = ["Spotlight", "Oral", "Award", "Best Paper", "Honorable Mention"];
// const renderHighlightedText = (text: string | undefined | null) => {
//   if (!text) return null;

//   // 创建正则表达式，( ) 捕获组是为了在 split 后保留分隔符（即关键词本身）
//   // 'gi' 表示全局匹配且忽略大小写
...
//       // 3. 如果匹配，渲染为红色加粗
//       return (
//         <span key={index} className="text-red-600 font-bold dark:text-red-400">
//           {part}
//         </span>
//       );
//     }
...
//   });
// };
export const renderHighlightedText = (text: string | undefined | null) => {
  if (!text) return null;

  // 逻辑：
  // 1. 匹配 Oral 或 Spotlight
  // 2. 匹配 "Best Paper Award", "Honorable Mention Award" 等
  //    这里我们假设 Award 前面可能有 1-3 个单词，避免把整个会议名都匹配进去。
  //    ([a-zA-Z]+\s+){0,3}Award -> 匹配 Award 前面 0 到 3 个单词
  const regex = /(Oral|Spotlight|([a-zA-Z-]+\s+){0,3}Award)/gi;
  ...
  return parts.map((part, index) => {
    ...
    // 排除掉正则捕获组产生的 undefined 或纯空格干扰（因为上面的正则有嵌套括号）
    // 简单的判断方法：只要包含 Oral, Spotlight 或 Award 且长度适中，就标红
...
export default function PublicationsList({ config, publications, embedded = false }: PublicationsListProps) {
    ...
    const filteredPublications = useMemo(() => {
        ...
        .sort((a, b) => b.year - a.year);   // 按年份从大到小排序
